...
public abstract class CommandHandler extends Command {
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, String[] args) {
        if (!enabled) {
            sender.sendMessage(plugin.getConfigManager().getComponent("command.disabled"));
            return true;
        }

        if (!hasPermission(sender)) {
            sender.sendMessage(plugin.getConfigManager().getComponent("command.no-permission"));
            return true;
        }

        if (requiresPlayer() && !(sender instanceof Player)) {
            sender.sendMessage(plugin.getConfigManager().getComponent("command.player-only"));
            return true;
        }
    ...
    protected void sendMessage(CommandSender sender, String key) {
        Component message = plugin.getConfigManager().getComponent(key);
